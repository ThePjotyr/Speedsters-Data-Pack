...
execute if dimension minecraft:the_nether if entity @s[nbt={Dimension:"minecraft:the_nether"}] run tag @s remove tp.speedfolrce.travelling
...
execute at @s[tag=!tp.speedforce.nether] unless predicate tp_speedforce:inside_speedforce unless entity @e[type=marker,tag=tp.speedforce.portal,distance=..2] run summon marker ~ ~1 ~ {NoGravity:1b,Tags:["tp.speedforce.portal"]}